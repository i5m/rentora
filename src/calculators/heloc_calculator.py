...
class HelocCalculator:
    def __init__(self, heloc_input: HelocInput):
        ...
        self.loan_length_years = heloc_input.loan_lenth_years
    ...
    def check_and_activate(self, year: int, current_home_value: float, remaining_mortgage: float) -> float:
        """
        ...
        if year == self.after_years and not self.is_active:
            ...
            if max_heloc > 0:
                ...
                if self.interest_rate > 0:
                    ...
                    self.monthly_payment = self.heloc_amount * (r * (1 + r)**n) / ((1 + r)**n - 1)
                else:
